...
public class CopyDataReader extends DataReader {
  ...
  public Batch nextBatch() {
    ...
    if (!config.isIS_ADD_ANOMALY()) {
      ...
      String type = config.getANOMALY_TYPE();
      // TODO 目前异常仅限于翻倍
      if (Objects.equals(type, "times")) {
      ...
      if (Objects.equals(type, "speed")) {
        for (int r = 1; r < records.size(); r++) {
          Record record = records.get(r);
          Record lastRecord = records.get(r - 1);
          record.setTimestamp(record.getTimestamp() + deltaTimeStamp);
          Random random = new Random();

          if (addAnomalies.get(r)) {
            List<Object> recordValues = record.getRecordDataValue();
            List<Object> lastRecordValues = lastRecord.getRecordDataValue();
            List<Object> anomalyValues = new ArrayList<>();
            double speed, randomTimes;

            // TODO
            for (int i = 0; i < sensors.size(); i++) {
              // 生成一个（2，5）或（-5，-2）的随机数，作为速度的倍数
              randomTimes = (3 * random.nextDouble() + 2) * (random.nextInt(2) * 2 - 1);
              ...
              switch (sensors.get(i).getSensorType()) {
                ...
                case BOOLEAN:
                  ...
                  break;
                case INT32:
                  speed = ( (Double) recordValues.get(i) - (Double) lastRecordValues.get(i))
                                  / (record.getTimestamp() - lastRecord.getTimestamp());

                  anomalyValues.add(
                          (Integer) lastRecordValues.get(i)
                                  + speed
                                  * randomTimes
                                  * (record.getTimestamp() - lastRecord.getTimestamp()));
                  break;
                case INT64:
                  speed = ( (Double) recordValues.get(i) - (Double) lastRecordValues.get(i))
                          / (record.getTimestamp() - lastRecord.getTimestamp());
                  anomalyValues.add(
                          (Long) lastRecordValues.get(i)
                                  + speed
                                  * randomTimes
                                  * (record.getTimestamp() - lastRecord.getTimestamp()));
                  break;
                case FLOAT:
                  float speed_f = ( (Float)recordValues.get(i) - (Float) lastRecordValues.get(i))
                          / (record.getTimestamp() - lastRecord.getTimestamp());
                  float randomTimes_f = (3 * random.nextFloat() + 2) * (random.nextInt(2) * 2 - 1);
                  anomalyValues.add(
                          (Float) lastRecordValues.get(i)
                                  + speed_f
                                  * randomTimes_f
                                  * (record.getTimestamp() - lastRecord.getTimestamp()));
                  break;
